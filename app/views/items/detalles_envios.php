<div class="container mt-5">
    <h1>Consulta y Actualización del Costo de Envío</h1>
    <p>Ingresa uno o varios ITEMs, separados por comas, para obtener su costo de envío y actualizar la base de datos.</p>

    <form action="/ventas/Items/procesarCostoEnvio" method="POST">
        <div class="mb-3">
            <label for="item_ids" class="form-label">ITEMs de publicaciones (Existentes en item_meli):</label>
            <input 
                type="text"
                class="form-control" 
                name="item_ids"             id="item_ids"                   placeholder="MLM3764133956,MLM2361209353" 
            >
        </div>
        <button type="submit" class="btn btn-primary">Actualizar Costo Envío</button>
    </form>

    <?php if (!empty($resultados)): ?>
        <hr class="mt-5">
        <h2>Resultados de la Actualización</h2>
        <ul class="list-group">
            <?php foreach ($resultados as $itemId => $resultado): ?> 
                <?php
                    $clase_alerta = ($resultado['success'] === true) ? 'list-group-item-success' : 'list-group-item-danger';
                ?>
                <li class="list-group-item d-flex justify-content-between align-items-center <?php echo $clase_alerta; ?>">
                    <div>
                        <strong>ID: <?php echo htmlspecialchars($itemId); ?></strong>
                        <br>
                        <span><?php echo htmlspecialchars($resultado['message']); ?></span>
                    </div>
                    <?php if ($resultado['success'] === true): ?>
                        <span class="badge bg-success">Éxito</span>
                    <?php else: ?>
                        <span class="badge bg-danger">Error</span>
                    <?php endif; ?>
                </li>
            <?php endforeach; ?>
        </ul>
    <?php endif; ?>
</div>